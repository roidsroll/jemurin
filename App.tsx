...
import { Plus, CloudSun, MoonStar, Info } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  return (
    <div className={`h-screen w-screen flex flex-col relative overflow-hidden transition-colors duration-1000 ${bgGradient}`}>
      ...
      >
        <div className="h-full relative" style={{ width: `${containerWidth}px` }}>
          ...
          {/* Hanging Notes Container */}
          <div className="absolute top-28 md:top-32 left-0 w-full h-full pt-2">
            {loading ? (
              ...
              notes.map((note, index) => (
                <HangingNote key={note.id} note={note} index={index} />
              ))
            )}
